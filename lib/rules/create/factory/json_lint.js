...
module.exports = function (options) {
  return function (context) {
    ...
    for (const check of checkList) {
      if (Array.isArray(check.document.syntaxErrors) && check.document.syntaxErrors.length > 0) {
      ...
      // strict defaults to true (undefined !== false); escape hatch is strict: false
      if (check.strict !== false && Array.isArray(check.commentErrors) && check.commentErrors.length > 0) {
        for (const error of check.commentErrors) {
          context.report({
            messageId: "errorMessage",
            data: { message: error.message },
            loc: {
